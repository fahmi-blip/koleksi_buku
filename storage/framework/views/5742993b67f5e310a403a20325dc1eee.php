...
<div style="display:grid;grid-template-columns:1fr 0.9fr;gap:20px;align-items:start;">
    <section style="border:1px solid rgba(148,163,184,0.2);background:rgba(15,23,42,0.72);border-radius:24px;padding:22px;">
        ...
        <h2 style="margin:10px 0 8px;">Pesanan <?php echo e($pesanan->order_code); ?></h2>
        <p style="margin:0 0 18px;color:#94a3b8;">Customer: <?php echo e($pesanan->nama); ?> | Vendor: <?php echo e($pesanan->vendor->nama_vendor); ?></p>
        ...
        <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;">
            <div style="padding:16px;border-radius:18px;background:rgba(255,255,255,.05);border:1px solid rgba(148,163,184,.16);">
                ...
                <strong style="display:block;margin-top:4px;text-transform:uppercase;"><?php echo e(str_replace('_', ' ', $pesanan->metode_bayar)); ?></strong>
            </div>
            <div style="padding:16px;border-radius:18px;background:rgba(255,255,255,.05);border:1px solid rgba(148,163,184,.16);">
                <div style="color:#94a3b8;font-size:.82rem;">Status pembayaran</div>
                <strong style="display:block;margin-top:4px;color:<?php echo e((int) $pesanan->status_bayar === 1 ? '#22c55e' : '#f59e0b'); ?>;"><?php echo e((int) $pesanan->status_bayar === 1 ? 'Lunas' : 'Menunggu pembayaran'); ?></strong>
            </div>
            <div style="padding:16px;border-radius:18px;background:rgba(255,255,255,.05);border:1px solid rgba(148,163,184,.16);">
                <div style="color:#94a3b8;font-size:.82rem;">Reference</div>
                <strong style="display:block;margin-top:4px;"><?php echo e($pesanan->payment_reference); ?></strong>
            </div>
            <div style="padding:16px;border-radius:18px;background:rgba(255,255,255,.05);border:1px solid rgba(148,163,184,.16);">
                <div style="color:#94a3b8;font-size:.82rem;">Total</div>
                <strong style="display:block;margin-top:4px;color:#22c55e;">Rp <?php echo e(number_format($pesanan->total, 0, ',', '.')); ?></strong>
            </div>
        </div>

        <?php if((int) $pesanan->status_bayar === 1): ?>
            <div style="margin-top:18px;padding:18px;border-radius:18px;background:linear-gradient(135deg,rgba(34,197,94,.14),rgba(56,189,248,.12));border:1px solid rgba(34,197,94,.25);text-align:center;">
                ...
                <p style="margin:0 0 14px;color:#94a3b8;">QR code ini berisi ID pesanan <strong><?php echo e($pesanan->idpesanan); ?></strong>.</p>
                ...
                <div style="margin-top:12px;font-weight:700;color:#e2e8f0;">ID Pesanan: <?php echo e($pesanan->idpesanan); ?></div>
            </div>
        <?php endif; ?>

        <?php if($paymentMethod === 'midtrans' && $snapToken): ?>
            <button id="pay-button" onclick="snapPay()" style="padding:14px 18px;width:100%;border:none;border-radius:16px;font-weight:700;background:linear-gradient(135deg,#22c55e,#16a34a);color:white;cursor:pointer;margin-top:18px;<?php echo e((int) $pesanan->status_bayar === 1 ? 'display:none;' : ''); ?>">Lanjutkan ke Pembayaran Midtrans</button>
            <form id="midtransConfirmForm" method="POST" action="<?php echo e(route('kantin.payment.confirm', $pesanan->order_code)); ?>" style="display:none;">
                <?php echo csrf_field(); ?>
            </form>
        <?php else: ?>
            <form method="POST" action="<?php echo e(route('kantin.payment.confirm', $pesanan->order_code)); ?>" style="margin-top:18px;">
                <?php echo csrf_field(); ?>
                <button type="submit" style="padding:14px 18px;width:100%;border:none;border-radius:16px;font-weight:700;background:linear-gradient(135deg,#22c55e,#16a34a);color:white;cursor:pointer;<?php echo e((int) $pesanan->status_bayar === 1 ? 'display:none;' : ''); ?>">Konfirmasi Pembayaran</button>
            </form>
        <?php endif; ?>
    </section>
            ...
            <div style="display:grid;gap:10px;">
                <?php $__currentLoopData = $pesanan->details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $detail): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div style="padding:14px;border-radius:16px;background:rgba(255,255,255,.04);border:1px solid rgba(148,163,184,.12);">
                        <strong style="display:block;"><?php echo e($detail->menu->nama_menu); ?></strong>
                        <small style="color:#94a3b8;"><?php echo e($detail->jumlah); ?> x Rp <?php echo e(number_format($detail->harga, 0, ',', '.')); ?></small>
                        <?php if($detail->catatan): ?>
                            <div style="margin-top:6px;color:#cbd5e1;"><?php echo e($detail->catatan); ?></div>
                        <?php endif; ?>
                    </div>
        ...
        <section style="border:1px solid rgba(148,163,184,0.2);background:rgba(15,23,42,0.72);border-radius:24px;padding:22px;">
            ...
            <?php if($paymentMethod === 'midtrans' && $snapToken): ?>
                ...
                <div style="padding:16px;border-radius:18px;background:linear-gradient(135deg,rgba(56,189,248,.14),rgba(34,197,94,.12));border:1px dashed rgba(148,163,184,.25);text-align:center;">
                    ...
                    <div style="font-size:1.1rem;font-weight:700;margin-top:6px;"><?php echo e($pesanan->payment_reference); ?></div>
                </div>
            <?php else: ?>
                ...
                <div style="padding:16px;border-radius:18px;background:linear-gradient(135deg,rgba(56,189,248,.14),rgba(245,158,11,.12));border:1px dashed rgba(148,163,184,.25);text-align:center;">
                    <div style="font-size:.82rem;color:#cbd5e1;"><?php echo e(strtoupper(str_replace('_', ' ', $pesanan->metode_bayar))); ?></div>
                    <div style="font-size:1.3rem;font-weight:700;margin-top:6px;"><?php echo e($pesanan->payment_reference); ?></div>
                </div>
            <?php endif; ?>
        </section>
    </aside>
    ...
    function snapPay() {
        snap.pay('<?php echo e($snapToken); ?>', {
            onSuccess: function(result) {
                ...
                window.location.href = '<?php echo e(route("kantin.payment.show", $pesanan->order_code)); ?>';
            },
...
<?php echo $__env->make('layouts.kantin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\koleksi-buku\resources\views\pages\kantin\payment.blade.php ENDPATH**/ ?>